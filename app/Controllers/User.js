									...
									try {
										UserInfo.create({'id':client.UID, 'name':name, 'joinedOn':new Date()}, function(errC, user){
											if (!!errC) {
												...
												Message.create({'uid': client.UID, 'title':'Thành Viên Mới', 'text':'contact telegram @thanhphong9595 - Cổng game hàng đầu Châu Á, chúc bạn chơi game vui vẻ, thắng lớn...', 'time':new Date()});
												GameState(client);
												client.red(data);
											}
										});
